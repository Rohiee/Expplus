...
if (! class_exists('bookingpress_services') ) {
    class bookingpress_services Extends BookingPress_Core
    {
        ...
        function bookingpress_add_service()
        {
            ...
            if ($service_duration_unit == 'd' && $bpa_service_duration > 7 ) { 
                $response            = array();
                $response['variant'] = 'error';
                $response['title']   = esc_html__('Error', 'bookingpress-appointment-booking');
                $response['msg']     = esc_html__('Service time duration cannot be greater than 7 days', 'bookingpress-appointment-booking') . '.';
                wp_send_json($response);
                die();
            }
            ...
            $service_description = ! empty($_POST['service_description']) ? wp_kses($_POST['service_description'], $bookingpress_allow_tag) : '';
            ...
            if (! empty($service_name) ) {
                ...
                if (! empty($service_id) ) {
                    if (empty($_POST['service_image']) ) { // phpcs:ignore WordPress.Security.NonceVerification
                        ...
                        if (! empty($_POST['service_image']) && ! empty($_POST['service_image_name']) ) { // phpcs:ignore WordPress.Security.NonceVerification
                            $service_img_url  = esc_url_raw($_POST['service_image']); // phpcs:ignore WordPress.Security.NonceVerification
                            $service_img_name = sanitize_file_name($_POST['service_image_name']); // phpcs:ignore WordPress.Security.NonceVerification
        ...
        function bookingpress_edit_service()
        {
            ...
            $service_id            = isset($_POST['edit_id']) ? intval($_POST['edit_id']) : ''; // phpcs:ignore WordPress.Security.NonceVerification
            $response              = array();
            ...
            if (! empty($service_id) ) {
                ...
                if (! empty($service) ) {
                    ...
                    $response['service_price']         = round($service['bookingpress_service_price'],$bookingpress_decimal_points);
