    ...
    class bookingpress_settings Extends BookingPress_Core
    {
        ...
        function bookingpress_delete_daysoff_details_func()
        {
            ...
            if (! empty($daysoff_date) ) {
                ...
                $wpdb->query( $wpdb->prepare( "DELETE FROM {$tbl_bookingpress_default_daysoff} WHERE bookingpress_dayoff_date LIKE %s", '%' . $daysoff_date . '%' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_default_daysoff is table name defined globally. False Positive alarm
            }
        ...
        function bookingpress_get_daysoff_details_func()
        {
            ...
            $daysoff_details         = $wpdb->get_results("SELECT * FROM {$tbl_bookingpress_default_daysoff}", ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_default_daysoff is table name defined globally. False Positive alarm
        ...
        function bookingpress_save_default_daysoff_details_func()
        {
            ...
            if (! empty($_REQUEST['daysoff_details']) ) {
                ...
                if (! empty($daysoff_title) ) {
                    ...
                    $dayoff_exist_or_not = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$tbl_bookingpress_default_daysoff} WHERE bookingpress_dayoff_date LIKE %s", '%'.$daysoff_date.'%' ), ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_default_daysoff is table name defined globally. False Positive alarm
        ...
        function bookingpress_send_test_email_func()
        {
            ...
            if (! empty($_REQUEST['notification_formdata']) ) {
                ...
                $smtp_password            = ! empty($_REQUEST['notification_formdata']['smtp_password']) ? $_REQUEST['notification_formdata']['smtp_password'] : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized --Reason - contains password
                $smtp_sender_name         = ! empty($_REQUEST['notification_formdata']['sender_name']) ? sanitize_text_field($_REQUEST['notification_formdata']['sender_name']) : '';
        ...
        function bookingpress_upload_company_avatar_func()
        {
            ...
            $bookingpress_fileupload_obj = new bookingpress_fileupload_class($_FILES['file']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        ...
        function bookingpress_setting_dynamic_vue_methods_func()
        {
            ...
            savegeneralSettingsData(){
                const vm = this
                vm.saveSettingsData('general_setting_form','general_setting')                
                vm.saveSettingsData('customer_setting_form','customer_setting')                
            },
