...
if (! class_exists('bookingpress_payment') ) {
    class bookingpress_payment Extends BookingPress_Core
    {
        ...
        function bookingpress_approve_appointment()
        {
            ...
            $payment_data                = $wpdb->get_row($wpdb->prepare("SELECT bookingpress_payment_log_id,bookingpress_appointment_booking_ref,bookingpress_customer_email,bookingpress_is_cart,bookingpress_order_id FROM {$tbl_bookingpress_payment_logs} WHERE bookingpress_payment_log_id = %d", $approve_payment_log_id), ARRAY_A);// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_payment_logs is table name defined globally. False Positive alarm
            ...
            if (! empty($get_payment_logs) ) {
                ...
                foreach ( $get_payment_logs as $payment_log_key => $payment_log_val ) {
                    ...
                    if ($appointment_date != '0000-00-00' ) {
                        $bookingpress_appointment_date = date($bookingpress_default_date_format, strtotime($appointment_date));
                    }
                    ...
                    $payment             = array(
                        ...
                        'payment_date'            => date($bookingpress_default_date_format, strtotime($payment_date)),
                        ...
                        'customer_firstname'      => $payment_log_val['bookingpress_customer_firstname'],
                        'customer_lastname'       => $payment_log_val['bookingpress_customer_lastname'],
